  ...
  if (!node) {
    return (
      <div className="flex flex-col items-center mx-2 sm:mx-4">
        <div className="w-16 h-16 rounded-full border-2 border-dashed border-slate-700 bg-slate-800/50 flex items-center justify-center mb-2 shadow-inner">
          <UserPlus size={20} className="text-slate-600" />
        </div>
        <div className="text-xs text-slate-600 font-medium">Empty Slot</div>
      </div>
    );
  }

  const isGreen = node.member?.greenStatus === 'ACTIVE';

  return (
    <div className="flex flex-col items-center relative">
      {/* Node Avatar Card */}
      <div className={elative group flex flex-col items-center z-10 bg-[#1E293B] rounded-xl p-3 border-2 shadow-lg transition-all duration-200 hover:scale-105 cursor-pointer w-32 sm:w-40 mx-2 sm:mx-4 }>
