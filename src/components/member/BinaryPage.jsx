...
const BinaryTreeHeader = ({ zoom, handleZoomOut, handleZoomIn, handleResetZoom }) => (
...
const BinaryTreeMetrics = ({ overview }) => (
...
const BinaryTreeNode = ({ node, depth = 0, selectedNode, setSelectedNode }) => {
  if (!node) return null;
...
const NodeDetailModal = ({ selectedNode, setSelectedNode }) => {
...
export const BinaryPage = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn w-full max-w-full overflow-hidden">
      ...
      <BinaryTreeMetrics overview={overview} />
      ...
      {/* Node Detail Modal */}
      <NodeDetailModal selectedNode={selectedNode} setSelectedNode={setSelectedNode} />
    </div>
  );
};
